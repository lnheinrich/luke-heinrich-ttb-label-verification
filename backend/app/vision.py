...
DEFAULT_VISION_MODEL = os.getenv("OPENAI_MODEL", "gpt-4.1-mini")
...
Set raw_text to null; do not transcribe the full label text.
...
class VisionService:
    # Extract label fields from an image using an injectable OpenAI client.
    def __init__(
        self,
    ...
    def extract_label(self, image_bytes: bytes, mime_type: str) -> ExtractedLabel:
        ...
        try:
            image_url, preprocess_result = image_bytes_to_data_url_with_metrics(image_bytes)
...
# Build the OpenAI client from the project-specific API key variable. Automatic
# retries are disabled so the request timeout is the real per-request budget.
...
# Convert arbitrary supported image bytes into an optimized inline data URL.
def image_bytes_to_data_url(image_bytes: bytes) -> str:
    image_url, _ = image_bytes_to_data_url_with_metrics(image_bytes)

    return image_url


# Convert image bytes into a data URL while preserving preprocessing metrics.
def image_bytes_to_data_url_with_metrics(
    image_bytes: bytes,
) -> tuple[str, ImagePreprocessResult]:
    preprocess_result = preprocess_image_with_metrics(image_bytes)
    encoded = base64.b64encode(preprocess_result.image_bytes).decode("ascii")

    return (f"data:image/jpeg;base64,{encoded}", preprocess_result)
...
# Read the parsed Pydantic object from an OpenAI structured response.
